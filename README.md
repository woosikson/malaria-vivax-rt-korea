...
This repository is intended to be deposited in a public archive and cited from
the manuscript's Data Availability Statement. To obtain a persistent identifier
(recommended by PLOS ONE), archive a tagged release on Zenodo (or a similar
service that mints DOIs) and cite that DOI.

Suggested Data Availability Statement:

> All code and input data needed to reproduce the figures are openly available
> on GitHub at <repository URL> and permanently archived on Zenodo at
> <https://doi.org/...>. The repository contains the raw input data, the analysis
> pipeline, and the figure-generation code; all figures can be regenerated by
